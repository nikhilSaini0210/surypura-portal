...
const ROAD_PHASES = [
  {
    phase: "Phase 1",
    desc: "4 km main village road to highway",
    status: "Completed",
    done: true,
  },
  {
    phase: "Phase 2",
    desc: "5 km inter-hamlet connectivity",
    status: "Completed",
    done: true,
  },
  {
    phase: "Phase 3",
    desc: "3 km school & hospital access road",
    status: "Completed",
    done: true,
  },
  {
    phase: "Phase 4",
    desc: "2 km agricultural market linkage road",
    status: "In Progress",
    done: false,
  },
];
...
const RoadsSection: FC = () => {
  return (
    <section
      id="roads"
      className="py-20 px-4 sm:px-6 lg:px-8 bg-gradient-to-b from-gray-950 to-orange-950/10 relative overflow-hidden"
    >
              ...
              {ROAD_PHASES.map((p, i) => (
                ...
                >
                  <div className="flex flex-col items-center">
                    <div
                      className={`w-4 h-4 rounded-full border-2 mt-3 flex-shrink-0 ${p.done ? "bg-green-500 border-green-400 shadow-green-500/50 shadow-lg" : "bg-yellow-500 border-yellow-400"}`}
                    />
                  ...
                  <div className="backdrop-blur-md bg-white/5 border border-white/10 rounded-2xl p-4 flex-1 mb-2 hover:border-orange-500/20 transition-colors duration-300">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-semibold ${p.done ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"}`}
                      >
                        {p.status}
                      </span>
                    </div>
