        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {PROGRAMS.map((program, i) => (
            <ScrollReveal key={program.id} delay={i * 0.1} direction="up">
              ...
              >
                <div
                  className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${program.color} flex items-center justify-center text-2xl mb-4 shadow-lg group-hover:scale-110 transition-transform duration-300`}
                >
                  {program.icon}
                </div>

                <Badge label={program.tag} color="green" />
                <h3 className="text-white font-bold text-xl mt-3 mb-1">
                  {program.title}
                </h3>
                <p className="text-green-400/70 text-sm font-hindi mb-3">
                  {program.titleHindi}
                </p>
